...
bool UpdateDrawMainMenu(Document *doc){
   ...
   if(showNewModal){
       ...
       // format selection
       if(GUIButton((Rectangle){mX + 20, mY + 60, 80,35}, "A4", selectedFormat == FORMAT_A4)) selectedFormat = FORMAT_A4;
       if(GUIButton((Rectangle){mX + 110, mY + 60, 80,35}, "A5", selectedFormat== FORMAT_A5)) selectedFormat = FORMAT_A5;
       if(GUIButton((Rectangle){mX + 200, mY + 60, 80,35}, "Letter", selectedFormat == FORMAT_LETTER)) selectedFormat = FORMAT_LETTER;
       if(GUIButton((Rectangle){mX+290, mY + 60, 90, 35}, "Custom", selectedFormat == FORMAT_CUSTOM)) selectedFormat = FORMAT_CUSTOM;

       // custom dimension UI
       if (selectedFormat == FORMAT_CUSTOM) {
            DrawText("Width:", mX + 20, mY + 120, 20, LIGHTGRAY);
            GUISlider((Rectangle){mX + 100, mY + 120, 200, 20}, &customW, 500.0f, 4000.0f);
            DrawText(TextFormat("%.0f", customW), mX + 320, mY + 120, 18, WHITE);
            ...
            DrawText(TextFormat("Canvas Size: %.0f x %.0f px", w, h), mX + 20, mY + 140, 20, LIGHTGRAY);
        }
       // buttons
