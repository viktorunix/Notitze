...
void InputHandler(Document *doc, Settings *settings, BindState *listeningForBind){
    if(!settings->showSettings && *listeningForBind == BIND_NONE){
        ...
        if(IsKeyPressed(settings->binds.keyUndo) && settings->binds.keyUndo != 0) UndoLastStrokes(&doc->pages[doc->activePage].layers[doc->pages[doc->activePage].activeLayer], doc->renderScale, doc->pressureEnabled);
...
void GUIHeaderDock(Document *doc, Settings *settings, Vector2 mousePos){
    ...
    curX += 80 + gap;
    if(GUIButton((Rectangle){curX, curY, 80, btnH}, "Undo", false)) UndoLastStrokes(&doc->pages[doc->activePage].layers[doc->pages[doc->activePage].activeLayer], doc->renderScale,doc->pressureEnabled);
    curX +=80 + gap;
